...
export class ChatService {
  static sendMessage = (lessonId: ILesson['id'], messages: SendMessageBody[]) => {
    const bodyMessages = [...messages]
    if (bodyMessages.length > 12) {
      bodyMessages.splice(1, bodyMessages.length - 10)
    }
    return httpClient.post('/api/lessons/', { messages: bodyMessages, lessonId })
  }
  ...
  static getSuggestion = (lessonId: ILesson['id'] | undefined, messages: SendMessageBody[]) => {
